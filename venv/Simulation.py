...
alpha = 0.02  # Coefficient influence de la température
beta = 0.02  # Coefficient influence du vent
theta = 0.01  # Coefficient influence des échanges au tour précédent
delta = 0.5  # Coefficient influence nouvelle énergie
epsilon = 0.5  # Coefficient influence conflit


def Home(ID, lock_queue, lock_count, queue, count, market_OK, clock_ok, temp, wind, weather_ok, num_policy,
         q_Term, queue_echanges):
    Money_Bal = 0
    Id = ID
    ...
    while clock_ok.wait(1.5 * delai):  # Attend le top au maximum pendant 1.5 fois le delai
        ...
        ###########Les réinitialisations se font ici, une fois que toutes les homes ont utilisé les flags (Si on les fait avant, certaines homes vont rester bloquées à l'attente du flag)

        if Energy_Bal > 0 and (
                num_policy == 1 or num_policy == 3):  # Si la Home est en surplus et qu'elle veut faire des échanges :

            with lock_queue:
                while (Energy_Bal != 0 and (
                        not queue.empty() or queue.qsize() != 0)):  # Tant qu'elle encore de l'énergie à donner et qu'il y a des demandes non traitées :
                    demande = queue.get()
                    ...
                    if abs(demande[1]) <= Energy_Bal:  # Si la demande est réalisable:
                        ...
                        queue.put([demande[0], demande[
                            1] + Energy_Bal])  # Donne son énergie restante et replace la demande mise à jour
        ...
        if (Energy_Bal < 0 and num_policy == 1) or (
                Energy_Bal != 0 and num_policy != 1):  # Une fois toutes les demandes traitées, si elles ont encore besoin d'énergies ou peuvent encore en donner
            with lock_queue:
...
def Market(queue, count, market_OK, clock_ok, temp, wind, mrkt_conn, weather_ok):
    ...
    while clock_ok.wait(1.5 * delai):  # Attend le top au maximum pendant 1.5 fois le delai
        ...
        internal = alpha * temp[1] + beta * wind[
            1] - theta * moy_exchange  # Calcul les effets internes(weather+echanges au tour précédent)
...
def Weather(temp, wind, clock_ok, weather_ok, weather_conn):
    ...
    while clock_ok.wait(1.5 * delai):  # Attend le top au maximum pendant 1.5 fois le delai
        ...
        temp[1] = 1 / ((temp[1] + 6.0) / (
                    15.0 + 6.0))  # On ne veut que des valeurs positives (donc +6) et on estime qu'une température <= à 15°C est idéal pour la consommation
...
def affiche(t, j, t2, m, w):
    print("################################################################## DAY", j,
          "##################################################################")
    print("")
    print("Temperature :", round(w[0], 3), "°C --> Modification :", round(alpha / ((w[0] + 6.0) / (15.0 + 6.0)), 3),
          "€/kW |  Wind :", round(w[1], 3), "km/h --> Modification :", round((beta * w[1] / 20.0), 3), "€/kW")
    print("Average purchases/sales day", j - 1, ":", round(m[4], 3), "kW --> Modification :", round((-theta * m[4]), 3),
          "€/kW")
    print("Price :", round(m[0], 3), "€/kW  |  Event :", m[1], "--> Modification Price :", round(m[2] * m[3], 3),
          "€/kW")
...
if __name__ == "__main__":
    ...
    for i in range(1, nmbHome + 1):
        ...
        h = Process(target=Home, args=(i, lockQueue, lockCount, queue, count, market_OK, clock_ok, temp, wind, weather_ok, num_policy, q, queue_echange),)
        h.start()
        if num_policy == 1:
